<?php
class EventPlus_Request extends EventPlus_Abstract_Request
{
    /**
     * Scheme for http
     *
     ...
     *     $browser = clsRequest::initial()->user_agent('browser');
     *
     ...
     * @uses    clsConfig::$config
     * @uses    clsRequest::$user_agent
     */
    public function user_agent($value)
    {
            ...
            {
                    // Load browsers
                    $oConfig = clsConfig::getSystemConfig('user_agents');
                    $userAgentsConfig = $oConfig->toArray();
                    $browsers = $userAgentsConfig['browser'];
                    ...
                    foreach ($browsers as $search => $name)
                    {
                            if (stripos($user_agent, $search) !== FALSE)
                            {
                                    ...
                                    if (preg_match('#'.preg_quote($search).'[^0-9.]*+([0-9.][0-9.a-z]*)#i', $user_agent, $matches))
                                    {
    ...
    public function __get($key)
    {
        switch (true) {
            ...
            case ($key == 'REQUEST_URI'):
                return $this->getRequestUri();
            case ($key == 'HTTP_REFERER'):
                return $this->getReferrer();
            case ($key == 'HTTP_USER_AGENT'):
    ...
    {
        switch (true) {
            case (isset($this->_params[$key]) && $this->_params[$key] != ''):
                return true;
            case (isset($_GET[$key]) && $_GET[$key] != ''):
                return true;
            case (isset($_POST[$key]) && $_POST[$key] != ''):
                return true;
            case (isset($_COOKIE[$key]) && $_COOKIE[$key] != ''):
                return true;
            case (isset($_SERVER[$key]) && $_SERVER[$key] != ''):
                return true;
            case (isset($_ENV[$key]) && $_ENV[$key] != ''):
                return true;
            default:
                return false;
        }
    ...
    public function getRequestUri()
    {
        ...
        $this->_requestUri = preg_replace('#//+#', '/', $this->_requestUri);
     ...
     * <img src="<?=$basePath?>/images/ice.png"/>
     *
     * @return string
     */
    ...
    public function setPathInfo($pathInfo = null)
    {
        if ($pathInfo === null) {
            ...
            if (null !== $this->_baseUrl
                && ((!empty($this->_baseUrl) && 0 === strpos($requestUri, $this->_baseUrl)) 
                    || empty($this->_baseUrl))
                    && false === ($pathInfo = substr($requestUri, strlen($this->_baseUrl)))
            ){ 
                // If substr() returns false then PATH_INFO is set to an empty string 
                $pathInfo = '';
            } elseif (null === $this->_baseUrl 
                    || (!empty($this->_baseUrl) && false === strpos($requestUri, $this->_baseUrl))
            ) { 
                $pathInfo = $requestUri; 
            }
    ...
    */
    ...
    {
            if ($key instanceof clsHTTP_Header)
            {
            ...
            if ($this->_header == null && $this->is_initial())
            {
                    // Lazy load the request headers
                    $this->_header = clsHTTP::request_headers();
            }
