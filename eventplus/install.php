...
function evrplus_table_upgrade($evrplus_new_tbl, $evrplus_old_tbl) {
    global $wpdb;
    if ($wpdb->get_var("SHOW TABLES LIKE '$evrplus_new_tbl'") != $evrplus_new_tbl) {
        $wpdb->query("CREATE TABLE IF NOT EXISTS " . $evrplus_new_tbl . " LIKE " . $evrplus_old_tbl);
        $wpdb->query("REPLACE INTO " . $evrplus_new_tbl . " SELECT * FROM " . $evrplus_old_tbl);
    }
}
...
// Attendee Table Copy Table, Replace Data, Add Colulmns        
    ...
    evrplus_table_upgrade($new_attendee_tbl, $old_attendee_tbl); //order - ()new_table,old_table)
...
// Event Table Copy Table, Replace Data, Add Colulmns        
    ...
    $old_event_tbl = $wpdb->prefix . "events_detail";
    evrplus_table_upgrade($new_event_tbl, $old_event_tbl); //order - ()new_table,old_table)
...
// Question Table Copy Table, Replace Data, Add Colulmns        
    ...
    evrplus_table_upgrade($new_question_tbl, $old_question_tbl); //order - ()new_table,old_table)
...
//        
    ...
    evrplus_table_upgrade($new_answer_tbl, $old_answer_tbl); //order - ()new_table,old_table)
...
//        
    ...
    evrplus_table_upgrade($new_category_tbl, $old_category_tbl); //order - ()new_table,old_table)
...
//        
    ...
    evrplus_table_upgrade($new_payment_tbl, $old_payment_tbl); //order - ()new_table,old_table)
        ...
        if ($old_events) {
            foreach ($old_events as $old_event) {
                ...
                $sql_data = array('%s', '%s', '%s', '%s', '%s', '%d', '%s', '%s', '%s', '%s',
                    '%s');
                $wpdb->insert($new_cost_tbl, $sql, $sql_data) or die(mysqli_error());
            }
        }
